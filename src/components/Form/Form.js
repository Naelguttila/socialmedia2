  ...
  return (
    <Paper className={classes.paper}>
      <form autoComplete="off" noValidate className={`${classes.root} ${classes.form}`} onSubmit={handleSubmit}>
        <Typography variant="h6">{currentId ? `Editing "${post.title}"` : 'Créé un Souvenir'}</Typography>
        ...
        <TextField name="tags" variant="outlined" label="HashTag" fullWidth value={postData.tags} onChange={(e) => setPostData({ ...postData, tags: e.target.value.split(',') })} />
        ...
        <Button variant="contained" color="secondary" size="small" onClick={clear} fullWidth>Réinitialiser</Button>
      </form>
    </Paper>
  );
};
